...
- Added per-icon imports such as `@weprograpp/iconsax-react/icons/EmojiHappy`.
...
- Preserved the root named-export API, all 1000 public icons, all six variants, props, refs,
  defaults, invalid-variant fallback, and legacy `dist/*` paths.
- Removed the unused `prop-types` dependency and obsolete build plugins.
- Added deterministic generation, exhaustive SVG-fidelity checks, consumer-build tests, package
  validation, and bundle-size budgets.
